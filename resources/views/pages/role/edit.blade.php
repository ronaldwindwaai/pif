                        ...
                        <div class="col-md-10">
                            <div class="form-group">
                                ...
                                <input id="name" type="text" class="form-control @error('name') is-invalid @enderror"  @error('name') aria-invalid="true" @enderror
                                    name="name" required placeholder="Name Role" value="{{ $data->name }}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-10">
                            ...
                            <select class="js-example-basic-multiple col-md-6" multiple="multiple" name="permission_id[]">
                                @foreach ($permissions as $permission)
                                    <option {{ in_array($permission->id, $role_permissions)?'selected' :'' }} value="{{ $permission->id }}">{{ $permission->name }}</option>
                                @endforeach
                            </select>
                            </div>
                        </div>
